...
class AnimeControllerIT {
    ...
    private static final DevDojoUser USER = DevDojoUser.builder()
            ...
            .password("{bcrypt}$2a$10$jtPiNlFNj6LsQddafyOvmeuH8F9GIalySeY5dqCu7ZJhwMpqMIBUe")
            .username("devdojo")
    ...
    static class Config {
        ...
        public TestRestTemplate testRestTemplateRoleAdminCreator(@Value("${local.server.port}") int port) {
            RestTemplateBuilder restTemplateBuilder = new RestTemplateBuilder()
                    .rootUri("http://localhost:" + port)
                    .basicAuthentication("jeff5m", "academy");
            return new TestRestTemplate(restTemplateBuilder);
        }
    }
    ...
    @Test
    @DisplayName("findByName returns empty list of animes when anime is not found")
    void findByName_ReturnsEmptyListOfAnimes_WhenAnimeIsNotFound() {
        devDojoUserRepository.save(USER);

        List<Anime> animes = testRestTemplateRoleUser.exchange(
                "/animes/find?name=asdf",
                HttpMethod.GET,
                null,
                new ParameterizedTypeReference<List<Anime>>() {
                }).getBody();
